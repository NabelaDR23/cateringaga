<?php
	...
	class mdlriwayatpinjaman extends db_connect{	
		...
		public function addriwayat($nama_alat, $jumlah){
			$stmt = $this->conn->prepare("INSERT INTO `tbriwayatpinjaman` (`nama_alat`, `jumlah`) 
		...
		public function postpengembalian($idalat, $idalatrusak, $tangganglperbaikikan, $nama_alat, $penanggungjawab, $jumlahpengembalian, $jumlahAlat, $updatejumlahbarang, $updatejumlahbarangrusak) {
			...
			$con = $this->conn;
			$resultUpdateAlat = mysqli_query($con,"update tbriwayatpinjaman set jumlah='$updatejumlahbarang' where id_alat='$idalat'");
		...
		public function read(){
			$stmt = $this->conn->prepare("SELECT * FROM `tbriwayatpinjaman` ORDER BY `id_alat` ASC") or die($this->conn->error);
			if($stmt->execute()){
		...
		public function hapus($postid){
			$con = $this->conn;
			mysqli_query($con ,"delete from tbriwayatpinjaman where id_alat = '$postid'");
		}


		public function edtfalat($postid){
			$con = $this->conn;
			$result = mysqli_query($con ,"select * from tbriwayatpinjaman where id_alat = '$postid'");
			return $result;
		}

		public function edtalat($id, $nama_alat, $jumlah){
			$con = $this->conn;
			$result = mysqli_query($con,"update tbriwayatpinjaman set nama_alat='$nama_alat', jumlah='$jumlah' where id_alat='$id'");
			return $result;
		}
